...
const audiences = [
  {
    title: "For Firms",
    description:
      "AI practice management for engineering, architecture, and surveying firms.",
    href: "/firms",
    audience: "firms",
    icon: (
      <svg
  ...
  },
  {
    ...
    icon: (
      ...
      >
        ...
        <circle cx="36" cy="14" r="6" fill="#C4883A" opacity="0.15" />
        <path d="M34 14l1.5 1.5L38 12" stroke="#C4883A" strokeWidth="1.5" />
      </svg>
    ),
  },
];
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {audiences.map((aud) => (
          <Link
            ...
            className="group bg-white border border-warm-gray rounded-2xl p-10 flex flex-col items-start gap-5 transition-all hover:shadow-lg hover:-translate-y-1"
          >
            {aud.icon}
            <h3 className="text-2xl font-serif text-navy">{aud.title}</h3>
            <p className="text-navy/60 leading-relaxed">{aud.description}</p>
            <span className="text-amber font-medium text-sm mt-auto group-hover:translate-x-1 transition-transform">
              Learn more &rarr;
            </span>
          </Link>
        ))}
